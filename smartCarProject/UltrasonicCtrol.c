...
volatile unsigned int distance_cm_last = 0;//当前距离
...
unsigned int GetDiff(unsigned int data1,unsigned int data2)
{
	if(data1 > data2)
		return data1 - data2;
	else
...
void Distance(void)
{
	...
	{
		ultick_5ms = 0;
	  DisableIRCtrol();//超声波测距是禁止遥控信号
		distance_cm_last = distance_cm;//记录上次的距离
		GetDistanceDelay();
		EnableTRCtrol();
	}
}
